...
$resourceGroupName = 'psconfasia2019_d1'
...
$sql = New-AzSqlServer -ServerName ( -join (1..12 | % { [char](Get-Random -Min 97 -Max 122) })) -SqlAdministratorCredentials $sqlAdmin -Location westeurope # India is not allowed to have SQL ;(
...
#>
$createTable = 'CREATE TABLE dbo.VersionEvent (PSVersion varchar(15) NOT NULL, OSType varchar(10) NOT NULL, ModuleVersion varchar(15) NOT NULL, Country nvarchar(255) NULL, City nvarchar(255) NULL, Date datetime NOT NULL);'
...
$aa = New-AzAutomationAccount -Name  (-join (1..12 | % { [char](Get-Random -Min 97 -Max 122) })) -Plan Basic
$aac = New-AzAutomationCredential -Name 'Database account' -Description 'Database account' -Value $sqlAdmin -AutomationAccountName $aa.AutomationAccountName
...
$code = {
    ...
    $Path = $pwd.Path
    $ctx = (Get-AzureRmStorageAccount -ResourceGroupName '%RESOURCEGROUP%' -Name %STORAGEACCOUNT% -ErrorAction SilentlyContinue).Context
    $key = (Get-AzureRmStorageAccountKey -ResourceGroupName '%RESOURCEGROUP%' -Name %STORAGEACCOUNT%)[0].Value
    ...
    $cString = "Server=tcp:%SQLHOST%.database.windows.net,1433;Initial Catalog=app1telemetry;Persist Security Info=False;User ID=$($Cred.UserName);Password=$($Cred.GetNetworkCredential().Password);MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;"
...
($code.ToString() -replace '%STORAGEACCOUNT%', $sa.StorageAccountName) -replace '%SQLHOST%',$sql.ServerName -replace '%RESOURCEGROUP%', $resourceGroupName | Set-Content .\runbook.ps1

# Lastly, import the runbook
$rbi = Import-AzAutomationRunbook -Path .\runbook.ps1 -Description 'Import telemetry into database' -Name RunLolaRun -Type PowerShell -AutomationAccountName $aa.AutomationAccountName -Force -Published
...
Set-PSFConfig -Module 'TelemetryHelper' -Name 'PowerOfTelemetry.ApplicationInsights.InstrumentationKey' -Value '$($ai.InstrumentationKey)' -PassThru | Register-PSFConfig
Set-PSFConfig -Module 'TelemetryHelper' -Name 'PowerOfTelemetry.OptIn' -Value `$false  -PassThru  | Register-PSFConfig
Set-PSFConfig -Module 'TelemetryHelper' -Name 'PowerOfTelemetry.OptInVariable' -Value 'PowerOfTelemetryOptIn'  -PassThru  | Register-PSFConfig
Set-PSFConfig -Module 'TelemetryHelper' -Name 'PowerOfTelemetry.RemovePII' -Value `$true  -PassThru  | Register-PSFConfig
"@ | Add-Content -Path .\PowerOfTelemetry\internal\configurations\configuration.ps1

'

. Import-ModuleFile "$($script:ModuleRoot)\internal\scripts\sendImportTelemetry.ps1"' | Add-Content -Path PowerOfTelemetry/internal/scripts/postimport.ps1
...
'@ | Set-Content -Path .\PowerOfTelemetry\internal\scripts\sendImportTelemetry.ps1

$split = if ($IsLinux -or $IsMacOS){':'} else {';'}
$null = Copy-Item -Path ".\PowerOfTelemetry" -Destination "$(($env:PSModulePath -split $split)[0])\PowerOfTelemetry" -Force -Recurse
$null = Copy-Item -Path ".\PowerOfTelemetry" -Destination "$(($env:PSModulePath -split $split)[1])\PowerOfTelemetry" -Force -Recurse
...
[Environment]::SetEnvironmentVariable('PowerOfTelemetryOptIn', 'yes', 'Machine')
Import-Module PowerOfTelemetry -Force
